...
///*****✅1.1定义代理协议
protocol HMWaterflowLayout_OtherDelegate:NSObjectProtocol{
...
class HMWaterflowLayout_Other: UICollectionViewLayout {
    ...
    ///*****✅1.2初始化代理协议
    weak var delegate:HMWaterflowLayout_OtherDelegate!
    
    /** 这个字典用来存储每一列最大的Y值(每一列的高度) */
    lazy var maxYDict = [Int: CGFloat]()
...
//        })
        ...
        return CGSizeMake(0, self.maxYDict[maxYColumn]! + self.sectionInset.bottom as CGFloat);
    ...
    override func layoutAttributesForItemAtIndexPath(indexPath: NSIndexPath) -> UICollectionViewLayoutAttributes? {
        ...
        for (column, maxY) in maxYDict {
            ...
            if maxY < maxYDict[minColumn] {
                // 找到更短的一列
                print("maxYDict[minColumn]?.floatValue\(maxYDict[minColumn])")
                minColumn = column
            }
        }
        ...
        if let max = self.maxYDict[minColumn]{
